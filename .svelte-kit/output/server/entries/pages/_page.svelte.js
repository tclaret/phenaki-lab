      ...
      if (store_get($$store_subs ??= {}, "$overlayVisible", overlayVisible) && store_get($$store_subs ??= {}, "$detectedCircle", detectedCircle)) {
        try {
          ...
          const equations = [
            ...
            `n = ${store_get($$store_subs ??= {}, "$detectedCount", detectedCount) || 0} frames`
          ];
          ...
          ctx.shadowBlur = 0;
          if (store_get($$store_subs ??= {}, "$detectedPositions", detectedPositions) && store_get($$store_subs ??= {}, "$detectedPositions", detectedPositions).length > 0) {
            const showIdx = Math.floor(radarTime * 2) % store_get($$store_subs ??= {}, "$detectedPositions", detectedPositions).length;
            const pos = store_get($$store_subs ??= {}, "$detectedPositions", detectedPositions)[showIdx];
            const ang = pos.angle;
          ...
          if (store_get($$store_subs ??= {}, "$detectedPositions", detectedPositions) && store_get($$store_subs ??= {}, "$detectedPositions", detectedPositions).length) {
            const posCount = store_get($$store_subs ??= {}, "$detectedPositions", detectedPositions).length;
            for (let i = 0; i < posCount; i++) {
              const p = store_get($$store_subs ??= {}, "$detectedPositions", detectedPositions)[i];
              const ang = p.angle;
