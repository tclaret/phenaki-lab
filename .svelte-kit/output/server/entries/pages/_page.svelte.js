...
function AnalyzerPanel($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    if (store_get($$store_subs ??= {}, "$detectedCircle", detectedCircle)) {
      $$renderer2.push("<!--[-->");
      $$renderer2.push(`<div${attr_class(`speed-control ${stringify("")} ${stringify("")} ${stringify(store_get($$store_subs ??= {}, "$isMobile", isMobile) ? "mobile" : "")}`, "svelte-14nyow8")}${attr_style(`left: ${stringify(Math.round((window.innerWidth - 200) / 2))}px; top: ${stringify(Math.round((window.innerHeight - 80) / 2))}px;`)}>`);
      if (store_get($$store_subs ??= {}, "$isMobile", isMobile)) {
        $$renderer2.push("<!--[-->");
