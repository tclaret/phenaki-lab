<script>
  ...
  import { countObjectsOnCircle } from "$lib/image/countOnCircle";
  import { detectCircle as detectCircleCV } from "$lib/image/detectCircle";
  import { sliceDisk } from "$lib/image/sliceDisk";
  ...
  async function runDetection() {
    ...
    try {
      ...
      if (!circle) {
        ...
        circle = { x: img.width / 2, y: img.height / 2, r: Math.min(img.width, img.height) / 2 * 0.9 };
      }
  ...
  async function saveGif() {
    ...
    try {
      ...
      const frames = sliceDisk(canvas, count, { circle, outputSize, margin: 1.05, zoom: 1, fps: 24, rotationSpeed: rotationSpeedValue, direction: directionValue });
  ...
  function saveOverlayPos() {
    try { localStorage.setItem(POS_KEY, JSON.stringify(overlayPos)); } catch (e) {}
  }
  ...
  function onOverlayPointerDown(e) {
    ...
    startPos = { left: overlayPos.left ?? Math.round((window.innerWidth - 200) / 2), top: overlayPos.top ?? Math.round((window.innerHeight - 80) / 2) };
...
<div class="panel">
  ...
  <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
    ...
    <button on:click={saveGif} disabled={exporting || !$playerCanvas}>{ exporting ? 'Exporting...' : 'Save GIF' }</button>
    <label style="display:flex;align-items:center;gap:6px;">
      <span style="font-size:12px;opacity:0.8;">Frames:</span>
      <input type="number" min="3" step="1" bind:value={optsGifCount} style="width:70px;padding:4px;border-radius:4px;border:1px solid #ccc;" />
    </label>
    <label style="display:flex;align-items:center;gap:6px;margin-left:6px;">
      <input type="checkbox" checked={_overlay} on:change={(e)=>overlayVisible.set(e.target.checked)} />
  ...
  <div style="margin-top:12px;">
    <div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap;">
      <div><strong>Current Speed:</strong> {($rotationSpeed).toFixed(0)}°/s | <em style="font-size:12px;opacity:0.7;">(Touch: drag up to speed up, down to slow down)</em></div>
      <div style="display:flex;gap:6px;align-items:center;">
        <label for="manual-speed-input" style="font-size:12px;">Type speed:</label>
        <input id="manual-speed-input" type="number" min="1" step="1" bind:value={manualSpeed} style="width:100px;padding:6px;border-radius:4px;border:1px solid #ccc;" />
    ...
    {#if _detectedCircle}
      <div><strong>Circle:</strong> x={Math.round(_detectedCircle.x)}, y={Math.round(_detectedCircle.y)}, r={Math.round(_detectedCircle.r)}</div>
    {/if}
  </div>
</div>
